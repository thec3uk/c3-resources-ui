// Sitemap code from: https://github.com/kentcdodds/kentcdodds.com
// simplified in some ways becuase I haven't included all the caching layers.
	...
	const rawSitemapEntries = (
		await Promise.all(
			Object.entries(remixContext.routeModules).map(async ([id, mod]) => {
				...
				let parent = parentId
					? remixContext.manifest.routes[parentId]
					: null;
